...
describe Parser do
  ...
  context 'terminal CPC' do
    ...
    it 'finds lib directory' do
      expect("#{Parser.lib_dir}/quests/quest_novo.txt").to be_truthy
    end
    ...
    it 'can access novo hash' do
      ...
      def Parser.file_to_parse
        "#{Parser.lib_dir}/quests/quest_novo.txt"
      end
    ...
    it 'finds another everything-line' do
      yaml_line =  { _name: ["03", "everything"] }
      quest_line = "-03 TANKER NAME : MT PERSEUS N"
      yaml_line.each do |key, value|
        quest_line.match(Parser.method(value[1]).call(value[0]))
        expect($1).to eq " MT PERSEUS N"
      end
    end
